...
@model function model(data)
    ...
    alphas_lens_0 ~ filldist(truncated(Normal(0, 1), -2, 2), 2)
    alphas_lens_1 ~ filldist(truncated(Normal(0, 1), -2, 2), 2)
    alphas_lens_2 ~ filldist(truncated(Normal(0, 1), -2, 2), 2)
    alphas_lens_3 ~ filldist(truncated(Normal(0, 1), -2, 2), 2)
    alphas_lens_4 ~ filldist(truncated(Normal(0, 1), -2, 2), 2)
...
folname = string("CosmoDC2_gcgc_Gibbs_wzdz_num",
    "_TAP_", TAP,
    "_init_ϵ1_", init_ϵ1, 
    "_init_ϵ2_", init_ϵ2,
)
...
if isdir(folname)
    ...
    if length(fol_files) != 0
        last_n = maximum([parse(Int,file[7:end-4]) for file in fol_files if occursin("chain", file)])
