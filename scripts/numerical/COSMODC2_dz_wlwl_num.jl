...
@model function model(data;
    ...
    dzs := SnWs[0, 2, 4, 6, 8]
    wzs := SnWs[1, 3, 5, 7, 9]
    source_0_zs = ((zs_k0-mu_k0)/wzs[0] + mu_k0 + dzs[0])
    source_1_zs = ((zs_k1-mu_k1)/wzs[1] + mu_k1 + dzs[1])
    source_2_zs = ((zs_k2-mu_k2)/wzs[2] + mu_k2 + dzs[2])
    source_3_zs = ((zs_k3-mu_k3)/wzs[3] + mu_k3 + dzs[3])
    source_4_zs = ((zs_k4-mu_k4)/wzs[4] + mu_k4 + dzs[4])
